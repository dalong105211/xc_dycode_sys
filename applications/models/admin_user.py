...
class User(db.Model, UserMixin):
    ...
    mail = db.Column(db.String(64), comment='邮件')
    enable = db.Column(db.Integer, default=0, comment='启用')
    dept_id = db.Column(db.Integer, comment='部门id')
    role_id = db.Column(db.Integer, comment='角色id')
    tool_func = db.Column(db.Integer, comment='func')
    creator = db.Column(db.String(20), comment='创建者')
    ...
    # role = db.relationship('Role', primaryjoin="admin_role", backref=db.backref('user'), lazy='dynamic')
