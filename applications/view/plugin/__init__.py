...
def enable():
    ...
    if plugin_folder_name:
        try:
            ...
            return fail_api(msg="Crash a error! Info: " + str(error))
        return success_api(msg="Enable success, reboot to make it work")
...
def disable():
    ...
    if plugin_folder_name:
        try:
            ...
            return fail_api(msg="Crash a error! Info: " + str(error))
        return success_api(msg="Disable success, reboot to make it work")
    return fail_api(msg="Data error")
...
def delete(plugin_folder_name):
    if plugin_folder_name in PLUGIN_ENABLE_FOLDERS:
        return fail_api(msg="You have to disable plugin first.")
    try:
        shutil.rmtree(os.path.abspath("plugins/" + plugin_folder_name))
        return success_api(msg="Remove success")
    except BaseException as error:
        return fail_api(msg="Remove fail, reason:" + str(error))
